...
$(document).on('submit', '.laravelComment-form', function(){
    ...
    var author = $('input#'+parent+'-input').val();
    var comment = $('textarea#'+parent+'-textarea').val();

    $.ajax({
         method: "get",
         url: "/laravellikecomment/comment/add",
         data: {parent: parent, comment: comment, item_id: item_id, author: author, content: comment},
         dataType: "json"
      })
      .done(function(msg){
        ...
        var newComment = '<div class="comment" id="comment-'+msg.id+'" style="display: initial;"><a class="avatar"><img src="'+msg.userPic+'"></a>' +
            '<div class="content"><a class="author">'+msg.userName+'</a><div class="metadata"><span class="date">Today at 5:97PM</span></div>' +
            '<div class="text">'+msg.comment+'</div><div class="actions"><a class="reply reply-button" data-toggle="'+msg.id+'-reply-form">Ответить</a></div>' +
            '<form class="ui laravelComment-form form" id="'+msg.id+'-reply-form" data-parent="'+msg.id+'" data-item="'+item_id+'" style="display: none;">' +
            '<div class="field"><input id="'+msg.id+'-input" pattern="^[A-ZА-ЯЁЇІЄҐ]{1}[A-Za-zА-Яа-яёЁЇїІіЄєҐґ]+\\s+[A-ZА-ЯЁЇІЄҐ]{1}[A-Za-zА-Яа-яёЁЇїІіЄєҐґ]+$" placeholder="Автор, по шаблону: <Фамилия> <Имя>">' +
            '<textarea id="'+msg.id+'-textarea"  placeholder="Текст" rows="2"></textarea></div><input type="submit" class="ui basic small submit button" value="Ответить">' +
            '</form></div>' +
            '<div class="ui threaded comments" id="'+item_id+'-comment-'+msg.id+'"></div></div>';
